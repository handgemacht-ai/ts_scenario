...
if git -C "$worktree_dir" ls-files --error-unmatch -- .worktree.env >/dev/null 2>&1; then
  git -C "$worktree_dir" update-index --skip-worktree -- .worktree.env >/dev/null 2>&1 || true
fi
